...
def firstRun():
	class MyWindow(xbmcgui.WindowXMLDialog):
		def __init__(self, *args, **kwargs):
			...
			self.msg   = "Atualmente voce nao possui nenhuma Build Instalada usando o %s.\n\nPara instalar uma Build: Clique em '[COLOR white]CLIQUE AQUI[/COLOR]', va em '[COLOR white]Add-ons[/COLOR]' Selecione '[COLOR white]Program Add-ons[/COLOR]' e execute o '[COLOR white]WIZARD Live Tv Tuga[/COLOR]', feito isso entre em  '[COLOR white]Builds[/COLOR]' e Escolha a sua '[COLOR white]Build[/COLOR]' no %s\n\nObrigado por utilizar o %s." % (ADDONTITLE,BUILDERNAME, ADDONTITLE)
...
def updateWindow(name='Testing Window', current='1.0', new='1.1', icon=ICON, fanart=FANART):
	class MyWindow(xbmcgui.WindowXMLDialog):
		def __init__(self, *args, **kwargs):
			...
			self.fanart = kwargs['fanart']
			self.msgupdate  = "Novo Update disponivel para a BUILD instalada:\n[COLOR %s]%s[/COLOR]\n\nBUILD ATUAL: v[COLOR %s]%s[/COLOR]\nBUILD NOVA: v[COLOR %s]%s[/COLOR]\n\n[COLOR %s]*ATUALIZE USANDO: Fresh install[/COLOR]" % (COLOR1, self.name, COLOR1, self.current, COLOR1, self.new, COLOR1)
