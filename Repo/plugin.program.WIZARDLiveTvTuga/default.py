...
def buildMenu():
	...
	if len(match) >= 1:
		...
		if adultcount > 0:
			...
			addFile('Atualmente nenhuma BUILD esta instalada pelo %s' % ADDONTITLE, '', icon=ICONBUILDS, themeit=THEME3)
	...
	if type == 'gui':
		...
		if theme == None:
			...
			if not themefile == 'http://' and wiz.workingURL(themefile) == True:
				...
				if len(themelist) > 0:
					if DIALOG.yesno(ADDONTITLE, "[COLOR %s]The Build [COLOR %s]%s[/COLOR] comes with [COLOR %s]%s[/COLOR] different themes" % (COLOR2, COLOR1, name, COLOR1, len(themelist)), "Would you like to install one now?[/COLOR]", yeslabel="[B][COLOR green]INSTALAR TEMA/UPDATE[/COLOR][/B]", nolabel="[B][COLOR red]Cancelar Tema[/COLOR][/B]"):
		...
		else: installtheme = DIALOG.yesno(ADDONTITLE, '[COLOR %s]Would you like to install the theme:' % COLOR2, '[COLOR %s]%s[/COLOR]' % (COLOR1, theme), 'for [COLOR %s]%s v%s[/COLOR]?[/COLOR]' % (COLOR1, name, wiz.checkBuild(name,'version')), yeslabel="[B][COLOR green]INSTALAR TEMA/UPDATE[/COLOR][/B]", nolabel="[B][COLOR red]Cancelar Tema[/COLOR][/B]")
		if installtheme:
...
def BuildList():
	...
	if not BUILDFILE == 'http://' and not BUILDFILE == '':
		...
		ThemeButton = pyxbmct.Button('[COLOR %s][B]Temas/Updates[/B][/COLOR]' % OTHER_BUTTONS_TEXT,focusTexture=FBUTTON,noFocusTexture=BUTTON)
...
def Installables():
	...
	AddonButton = pyxbmct.Button('[COLOR %s][B]Torrents[/B][/COLOR]' % OTHER_BUTTONS_TEXT,focusTexture=EXIT,noFocusTexture=BUTTON)
	...
	ROMButton = pyxbmct.Button('[COLOR %s][B]Wizards[/B][/COLOR]' % OTHER_BUTTONS_TEXT,focusTexture=EXIT,noFocusTexture=BUTTON)
...
MaintButton = pyxbmct.Button('[COLOR %s][B]Limpeza[/B][/COLOR]' % MAIN_BUTTONS_TEXT,focusTexture=FBUTTON,noFocusTexture=BUTTON)
...
InstallablesButton = pyxbmct.Button('[COLOR %s][B]Instalaveis[/B][/COLOR]' % MAIN_BUTTONS_TEXT,focusTexture=FBUTTON,noFocusTexture=BUTTON)
